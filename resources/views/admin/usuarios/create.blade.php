        ...
        <div class="col-lg-10">
            <div class="card-dark">
                ...
                <div class="card-body-dark">
                    <form method="POST" action="{{ route('admin.usuarios.store') }}" id="createUserForm" autocomplete="off">
                        @csrf
...
@endphp
                            ...
                            <div class="col-md-6" id="campo_estado_general" style="display: none;">
                            ...
                            <!-- Campo estado profesor (solo para tipo profesor) -->
                            <div class="col-md-6" id="campo_estado_profesor" style="display: none;">
                                <label for="estado_profesor" class="form-label-dark">
                                    <i class="fas fa-user-tie me-2"></i>
                                    Estado del Profesor
                                    <span class="required-mark">*</span>
                                </label>
                                <select class="form-select-dark @error('estado_profesor') is-invalid @enderror"
                                        id="estado_profesor"
                                        name="estado_profesor">
                                    <option value="">Seleccione estado</option>
                                    <option value="titular" {{ old('estado_profesor') == 'titular' ? 'selected' : '' }}>Titular</option>
                                    <option value="interino" {{ old('estado_profesor') == 'interino' ? 'selected' : '' }}>Interino</option>
                                    <option value="suplente" {{ old('estado_profesor') == 'suplente' ? 'selected' : '' }}>Suplente</option>
                                    <option value="licencia" {{ old('estado_profesor') == 'licencia' ? 'selected' : '' }}>Licencia</option>
                                    <option value="jubilado" {{ old('estado_profesor') == 'jubilado' ? 'selected' : '' }}>Jubilado</option>
                                    <option value="suspendido" {{ old('estado_profesor') == 'suspendido' ? 'selected' : '' }}>Suspendido</option>
                                </select>
...
<script>
    ...
    document.getElementById('tipo_usuario').addEventListener('change', function() {
        ...
        const campoEstadoProfesor = document.getElementById('campo_estado_profesor');
        ...
        if (this.value === 'estudiante') {
            ...
            campoEstadoProfesor.style.display = 'none';
            configurarCamposRepresentante(true);

            // ✅ NUEVO: Bloquear cambio de tipo_usuario cuando es estudiante
            this.disabled = true;
            // Agregar un campo oculto para enviar el valor
            if (!document.getElementById('tipo_usuario_hidden')) {
                const hiddenInput = document.createElement('input');
                hiddenInput.type = 'hidden';
                hiddenInput.id = 'tipo_usuario_hidden';
                hiddenInput.name = 'tipo_usuario';
                hiddenInput.value = 'estudiante';
                this.parentElement.appendChild(hiddenInput);
            }
            // Agregar mensaje informativo
            if (!document.getElementById('tipo_locked_msg')) {
                const msg = document.createElement('small');
                msg.id = 'tipo_locked_msg';
                msg.className = 'form-text-dark';
                msg.style.color = '#10b981';
                msg.innerHTML = '<i class="fas fa-lock me-1"></i>El tipo de usuario estudiante no se puede cambiar';
                this.parentElement.appendChild(msg);
            }
        } else if (this.value === 'profesor') {
            // Mostrar campo específico para profesores
            camposEstudiante.style.display = 'none';
            campoEstadoGeneral.style.display = 'none';
            campoEstadoProfesor.style.display = 'block';
            document.getElementById('estado_profesor').required = true;
            configurarCamposRepresentante(false);
        } else {
            // Administrador u otro tipo
            camposEstudiante.style.display = 'none';
            campoEstadoGeneral.style.display = 'block';
            campoEstadoProfesor.style.display = 'none';
            document.getElementById('estado_profesor').required = false;
            configurarCamposRepresentante(false);
        }
    });
    ...
    // Validación para teléfono (solo números, espacios, guiones, paréntesis y +)
